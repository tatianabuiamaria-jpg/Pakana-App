        ...
        {!loading && listings.map((l) => (
          <button key={l.id} style={styles.listingCard} onClick={() => onOpen(l)}>
            <div style={styles.listingThumb}>
              {l.photo_url ? (
                <img src={l.photo_url} alt={l.title} style={styles.listingThumbImg} />
              ) : (
  ...
  return (
    <div style={styles.screen}>
      ...
      <div style={styles.detailHero}>
        {listing.photo_url ? (
          <img src={listing.photo_url} alt={listing.title} style={styles.detailHeroImg} />
        ) : (
...
function PostForm({ onCancel, onSubmit }) {
  ...
  function handlePhotoSelect(e) {
    ...
    if (!file) {
      alert("No file detected from picker.");
      return;
    }
    try {
      setPhotoFile(file);
  ...
  async function uploadPhoto(file) {
    ...
    const res = await fetch(`${SUPABASE_URL}/storage/v1/object/listing-photos/${path}`, {
      method: "POST",
      headers: {
        apikey: SUPABASE_ANON_KEY,
        Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
  ...
  async function handleSubmit() {
    ...
    if (photoFile) {
      ...
      try {
        ...
        setUploading(false);
        alert("Couldn't upload the photo — try again, or post without one.");
        return;
      }
      setUploading(false);
    }

    onSubmit({ ...form, photo_url });
  }
  ...
  return (
    <div style={styles.screen}>
      ...
      {photoPreview ? (
        <label style={styles.photoPreviewWrap}>
          <img src={photoPreview} alt="Listing preview" style={styles.photoPreviewImg} />
          <span style={styles.photoChangeLabel}>Tap to change</span>
          <input
            type="file"
            accept="image/*"
