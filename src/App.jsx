...
import { Search, Plus, MapPin, Phone, MessageCircle, ChevronLeft, Camera, X, Loader2 } from "lucide-react";
...
function PostForm({ onCancel, onSubmit, defaultPhone }) {
  ...
  async function handleSubmit() {
    ...
    if (!form.title.trim() || !form.price.trim() || !form.area.trim() || digitsOnly.length < 7) {
      return;
    }
    ...
    onSubmit({ ...form, phone: digitsOnly, photo_url: photo_urls[0] || null, photo_urls });
  }

  const phoneDigits = form.phone.replace(/\D/g, "");
  const valid = form.title.trim() && form.price.trim() && form.area.trim() && phoneDigits.length >= 7;
