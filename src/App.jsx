...
import { Search, Plus, MapPin, Phone, MessageCircle, ChevronLeft, Camera, X, Loader2, Heart, ArrowUpDown, Bookmark, Share2, Pencil } from "lucide-react";
...
export default function App() {
  ...
  // Handle deep links — if the URL has ?listing=<id>, auto-open that listing
  // once listings are loaded. This is what makes share links work.
  useEffect(() => {
    if (listings.length === 0) return;
    const params = new URLSearchParams(window.location.search);
    const deepId = params.get("listing");
    if (!deepId) return;
    const target = listings.find((l) => l.id === deepId);
    if (target) {
      setActiveListing(target);
      setScreen("detail");
      // Clean up the URL so it doesn't re-trigger if user navigates back
  ...
  async function updateListing(id, updatedListing) {
    try {
      await supabaseRest.updateById("listings", id, {
        title: updatedListing.title,
        category: updatedListing.category,
        area: updatedListing.area,
        price: updatedListing.price,
        description: updatedListing.desc,
        phone: updatedListing.phone,
        photo_url: updatedListing.photo_url || null,
        photo_urls: updatedListing.photo_urls || [],
    ...
    <div style={styles.appShell}>
      <div style={styles.phoneFrame}>
        <div style={styles.content}>
          ...
          {screen === "post" && (
            <PostForm
              onCancel={() => { setEditingListing(null); setScreen(editingListing ? "mine" : "browse"); }}
              onSubmit={editingListing
                ? (data) => updateListing(editingListing.id, data)
                : addListing
              }
    ...
    <div style={styles.screen}>
      ...
      <div style={styles.listingList}>
        ...
        {!loading && listings.map((l) => {
          ...
          const daysSincePosted = Math.floor((Date.now() - new Date(l.created_at)) / (1000 * 60 * 60 * 24));
          const isStale = !isSold && daysSincePosted >= 14;
          return (
              ...
              <button style={styles.listingCardMain} onClick={() => onOpen(l)}>
                ...
                <div style={styles.listingInfo}>
                  ...
                  <div style={styles.listingFooter}>
                    ...
                    <span style={{
                      ...styles.listingTime,
                      ...(isStale ? styles.listingTimeStale : {}),
                    }}>
                      {isStale ? `${daysSincePosted}d ago · may be old` : l.posted}
                    </span>
                  </div>
                </div>
              </button>
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: listing.title,
          text: `${listing.title} — N$${listing.price} in ${listing.area}`,
          url,
        });
      } catch (e) {
    ...
    <div style={styles.screen}>
      <div style={styles.detailTopRow}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button style={styles.detailSaveBtn} onClick={shareListing} aria-label="Share">
            <Share2 size={16} color={COLORS.inkSoft} strokeWidth={2} />
          </button>
...
function PostForm({ onCancel, onSubmit, defaultPhone, editingListing }) {
  ...
  const [form, setForm] = useState(() => {
    if (editingListing) {
      ...
      return {
        ...
        category: editingListing.category || "home",
        area: editingListing.area || AREAS[1],
        areaIsOther: editingListing.area && !AREAS.includes(editingListing.area),
        price: editingListing.price || "",
        desc: editingListing.description || editingListing.desc || "",
        phone: editingListing.phone || defaultPhone || "",
      };
    }
    return {
  ...
  const [photoFiles, setPhotoFiles] = useState([]); // array of { file, preview } — only NEW photos to upload
  ...
  async function handleSubmit() {
    ...
    onSubmit({
      ...
      // If new photos were picked, use those; otherwise keep existing ones
      photo_url: photo_urls.length > 0 ? photo_urls[0] : (isEditing ? (editingListing.photo_url || null) : null),
      photo_urls: photo_urls.length > 0 ? photo_urls : (isEditing ? (editingListing.photo_urls || []) : []),
    });
  }
  ...
  return (
    <div style={styles.screen}>
      <div style={styles.postHeaderRow}>
        ...
        <h2 style={styles.postHeaderTitle}>{isEditing ? "Edit listing" : "New listing"}</h2>
        <div style={{ width: 36 }} />
      </div>
      ...
      >
        {uploading ? "Saving..." : isEditing ? "Save changes" : "Post listing"}
      </button>
    </div>
  );
}
...
function MyListings({ listings, onOpen, onRemove, onMarkSold, onConfirmAvailable, onEdit }) {
            ...
            <div key={l.id} style={styles.myListingCardWrap}>
              ...
              <div style={styles.myListingActions}>
                <button
                  ...
                  onClick={() => onEdit(l)}
                >
                  <Pencil size={11} style={{ marginRight: 4, verticalAlign: "middle" }} />Edit
                </button>
                <button
                  style={{ ...styles.myListingActionBtn, borderLeft: `1px solid ${COLORS.border}` }}
...
const styles = {
  ...
  listingTime: {},
  listingTimeStale: { color: COLORS.ochre, fontWeight: 600 },
